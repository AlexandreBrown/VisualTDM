...
class AddGoalLatentReached(Transform):
    def __init__(self, goal_reached_epsilon: float, terminate_when_goal_reached: bool, distance_type: str):
        super().__init__(in_keys=["pixels_latent", "goal_latent", "planning_horizon"], out_keys=["goal_latent_reached", f"goal_latent_{distance_type}_distance"])
    ...
    def _call(self, tensordict: TensorDict):
        ...
        tensordict[self.out_keys[0]] = (1 - goal_not_reached).type(torch.bool)
        tensordict[self.out_keys[1]] = distance.type(torch.float32)
        return tensordict
    ...
    def transform_observation_spec(self, observation_spec: TensorSpec) -> TensorSpec:
        ...
        observation_spec[self.out_keys[1]] = UnboundedContinuousTensorSpec(
            shape=(1,),
            device=observation_spec.device,
            dtype=torch.float32
        )

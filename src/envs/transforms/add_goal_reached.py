...
class AddGoalLatentReached(Transform):
    def __init__(self, goal_reached_epsilon: float):
        super().__init__(in_keys=["pixels_latent", "goal_latent", "planning_horizon"], out_keys=["done", "goal_latent_reached", "goal_latent_l2_distance"])
    ...
    def _call(self, tensordict: TensorDict):
        ...
        norm = compute_distance(norm_type='l2', obs_latent=next_obs_latent, goal_latent=goal_latent).mean()

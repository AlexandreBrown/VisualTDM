...
def create_ant_maze_env(device: torch.device) -> EnvBase:
...
def create_point_maze_env(device: torch.device) -> EnvBase:
    env = gym.make('PointMaze_UMazeDense-v3', render_mode='rgb_array')
    env = GymWrapper(env, from_pixels=True, pixels_only=False, device=device)
    return env


def create_franka_kitchen_env(device: torch.device) -> EnvBase:
